...
P_t = function(a){
  sapply(t_grid, function(t){
                    exp(-w * t) + sum(zeta(a) * sapply(u, function(ui) fW(a, t, ui)))
                  })
}
...
k = function(a){
  (1 + a*t(u)%*%Q%*%b) / (sinh(a*t(u))%*%Q%*%b)
}
f = function(a){
  w + (a/(1 - k(a)*exp(a*v)))
}

a = optim(par = 0.05, function(a) {abs(f(a)[60] - w)}, lower = 0.05)
length(t_grid)
plot(t_grid, curve(a), col = "pink", main = "taux spot (zero coupon)")
abline(a = ufr, b = 0, col = "red")
abline(v = 60, col = "red")
#points(u, data$spot, col = "blue", pch = 3)

plot(t_grid, f(a), col = "pink", ylim = c(min(f), 0.034), main = "taux forward (zero coupon)")
...
optim(par = 0.05, function(a) {abs(f(a)[60] - w)}, lower = 0.05)

plot(t_grid, curve, col = "pink", main = "taux spot (swaps)")
abline(a = ufr, b = 0, col = "red")
abline(v = 60, col = "red")
